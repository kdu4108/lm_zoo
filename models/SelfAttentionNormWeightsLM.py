...
class SelfAttentionNormWeightsLM(nn.Module):
    """
    This model implements self-attention, including projection to keys/value/queries AND normalizing weights after dot product.
    ...
    def __init__(self, vocab_sz: int, emb_sz: int = 16):
        super(SelfAttentionNormWeightsLM, self).__init__()
    ...
    def forward(self, x: torch.LongTensor) -> torch.FloatTensor:
        ...
        # Normalize similarity scores by dividing by the sqrt(keys_emb_sz) to reduce variance to 1.
        # Why? If the variance is high, then softmax will make the weights probability distribution "pointier" than it should be,
        # and therefore the attention mechanism will focus too much on one word.
